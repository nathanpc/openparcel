 ...
 */
function errorCheck() {
	const errBlock = document.querySelector(".c-tracking-result--status-shipment-undefined");

	if (errBlock !== null) {
		const errMessage = document.querySelector(".c-tracking-result--info .c-tracking-result-message--content").innerText;
		...
		if (errMessage.includes("tracking attempt was not successful")) {
			...
			return OpenParcel.error.create(OpenParcel.error.codes.InvalidTrackingCode);
		}
 ...
 *
 * @return OpenParcel address object.
 */
 ...
 * @param {Date}    date Date and time that this update occurred.
 *
 * @returns OpenParcel tracking history item object.
 */
function parseHistoryItem(item, date) {
